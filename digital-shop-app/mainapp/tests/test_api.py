...
class CartServiceTestCase(TestCase):
    ...
    def test_get_cart(self):
        ...
        self.assertEqual(res2.data[consts.DETAIL_KEY], consts.CART_ALREADY_EXISTS)
...
class CartItemServiceTestCase(TestCase):
    ...
    # Initial data
    def setUp(self) -> None:
        self.superuser = CustomUser.objects.create_superuser(email="super1@gmail.com", username="super1", password="123")
        self.user = CustomUser.objects.create_user(email="test1@gmail.com", username="test1", password="1234")
        self.cart = Cart.objects.create(user=self.user)
        self.category = Category.objects.create(name="Test category 1", verbose="test-category-1")
        self.product = Product.objects.create(category=self.category, created_by=self.superuser, title="Test title 1", price=40.0, quantity=20)
        self.product2 = Product.objects.create(category=self.category, created_by=self.superuser, title="Test title 2", price=120.0, quantity=40)
        self.cart_item1 = CartItem.objects.create(quantity=1, cart=self.cart, product=self.product)
        self.cart_item2 = CartItem.objects.create(quantity=3, cart=self.cart, product=self.product2)

    def test_add_execute(self):
        self.assertEqual(CartItem.objects.count(), 2)
        new_product = Product.objects.create(category=self.category, created_by=self.superuser, title="Test title 3", price=20.4, quantity=6)
        request = RequestFactory().request()
        request.data = {
    ...
    def test_post_purchase_execute(self):
        ...
        request = RequestFactory().request()
        # Perhaps this user should be authenticated (look at func)
        request.user = self.user
        request.data = {
            consts.CART_ID_POST_KEY: self.cart.id,
        ...
        self.assertEqual(res.cookies[consts.USER_CART_ID_COOKIE_NAME].value, str(Cart.objects.filter(is_archived=False, user=self.user).first().id))
        self.assertTrue(Cart.objects.filter(id=self.cart.id).first().is_archived)

        # 2nd case: with only non-user cart
        total_price = 40
        new_cart = Cart.objects.create()
        new_cart_item = CartItem.objects.create(quantity=1, cart=new_cart, product=self.product)
        ...
        self.assertEqual(res2.cookies[consts.NON_USER_CART_ID_COOKIE_NAME].value, str(Cart.objects.filter(is_archived=False, user=None).first().id))
